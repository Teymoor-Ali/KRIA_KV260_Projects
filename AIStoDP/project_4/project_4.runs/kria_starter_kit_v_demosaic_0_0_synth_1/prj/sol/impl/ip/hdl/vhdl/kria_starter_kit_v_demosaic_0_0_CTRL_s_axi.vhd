...
entity kria_starter_kit_v_demosaic_0_0_CTRL_s_axi is
generic (
...
end entity kria_starter_kit_v_demosaic_0_0_CTRL_s_axi;
...
architecture behave of kria_starter_kit_v_demosaic_0_0_CTRL_s_axi is
    ...
    constant ADDR_BAYER_PHASE_DATA_0 : INTEGER := 16#28#;
    constant ADDR_BAYER_PHASE_CTRL   : INTEGER := 16#2c#;
    ...
    signal int_bayer_phase     : UNSIGNED(15 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_BAYER_PHASE_DATA_0 =>
                        rdata_data <= RESIZE(int_bayer_phase(15 downto 0), 32);
                    when others =>
                        NULL;
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    bayer_phase          <= STD_LOGIC_VECTOR(int_bayer_phase);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_bayer_phase(15 downto 0) <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_BAYER_PHASE_DATA_0) then
                    int_bayer_phase(15 downto 0) <= (UNSIGNED(WDATA(15 downto 0)) and wmask(15 downto 0)) or ((not wmask(15 downto 0)) and int_bayer_phase(15 downto 0));
                end if;
            end if;
        end if;
    end process;
